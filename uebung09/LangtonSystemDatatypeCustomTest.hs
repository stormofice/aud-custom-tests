...
main = do
    ...
    assert "getX/getY with one step" (getX (Step(New E)), getY (Step(New S))) (0, 0)
    assert "getX/getY with two steps" (getX (Step(Step(New W))), getY (Step(Step(New W)))) (1, 1)
    assert "getX/getY with three steps" (getX (Step(Step(Step(New N)))), getY (Step(Step(Step(New N))))) (0, -1)
    ...
    assert "getCol with two steps" (getCol (Step(Step(New S))) 0 0, getCol (Step(Step(New S))) 0 (-1), 
                                        getCol (Step(Step(New S))) 1 (-1)) (BLACK, WHITE, WHITE)
    putStrLn "===================================================="
